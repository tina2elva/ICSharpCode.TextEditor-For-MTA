...
namespace ICSharpCode.TextEditor.Document
{
	public class FoldingManager
	{
		...
		public void UpdateFoldings(List<FoldMarker> FoldMarkers)
		{
			int oldFoldingsCount = foldMarker.Count;
			lock (this)
			{
				if (FoldMarkers != null && FoldMarkers.Count != 0)
				{
					FoldMarkers.Sort();
					if (foldMarker.Count == FoldMarkers.Count)
					{
						for (int i = 0; i < foldMarker.Count; ++i)
						{
							FoldMarkers[i].IsFolded = foldMarker[i].IsFolded;
						}
						foldMarker = FoldMarkers;
					}
					else
					{
						for (int i = 0, j = 0; i < foldMarker.Count && j < FoldMarkers.Count; )
						{
							int n = FoldMarkers[j].CompareTo(foldMarker[i]);
							if (n > 0)
							{
								++i;
							}
							else
							{
								if (n == 0)
								{
									FoldMarkers[j].IsFolded = foldMarker[i].IsFolded;
								}
								++j;
							}
						}
				...
				{
					foldMarker = FoldMarkers;
					foldMarkerByEnd = new List<FoldMarker>(FoldMarkers);
