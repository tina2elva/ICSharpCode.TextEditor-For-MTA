...
//			((DefaultWorkbench)WorkbenchSingleton.Workbench).UpdateToolbars();
		...
		static void StartSTAThread(Action action, bool isSync = false)
		{
            System.Threading.Thread thread = new System.Threading.Thread(new System.Threading.ThreadStart(action));
			...
			for (int i = 0;; i++) {
				try {
					...
					StartSTAThread(() =>
					{
						...
						if (data == null)
							return;
						fullLine = data.GetDataPresent(LineSelectedType);
						isUnicodeText = data.GetDataPresent(DataFormats.UnicodeText);
						if (isUnicodeText)
